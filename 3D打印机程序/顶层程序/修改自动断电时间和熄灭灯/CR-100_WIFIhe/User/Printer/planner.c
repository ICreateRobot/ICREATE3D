...
{
  if(acceleration != 0) return((target_rate*target_rate-initial_rate*initial_rate) / (2.0*acceleration));
  else return 0.0;  // acceleration was 0, set acceleration distance to 0
}
...
{
  if(acceleration!=0) return((2.0*acceleration*distance-initial_rate*initial_rate+final_rate*final_rate) / (4.0*acceleration));
  else return 0.0;  // acceleration was 0, set intersection distance to 0
}
...
void planner_reverse_pass_kernel(block_t *previous, block_t *current, block_t *next)
{
  ...
  if(next)
  {
    ...
    if(current->entry_speed != current->max_entry_speed)
    {
      ...
      else current->entry_speed = current->max_entry_speed;
...
void getHighESpeed(void)
{
  ...
  if(Heater_GetTargetCelsius(Heater_Head0)+2 < autotemp_min) return;
  high = 0.0;
  block_index = block_buffer_tail;
  while(block_index != block_buffer_head) 
  {
    if((block_buffer[block_index].steps_x!=0) || (block_buffer[block_index].steps_y!=0) || (block_buffer[block_index].steps_z!=0))
    {
      float se;
  ...
  oldt = t;
  Heater_SetTargetCelsius(t, Heater_Head0);
}
#endif
...
void check_axes_activity(void)
{
  ...
  if((DISABLE_X) && (x_active==0)) Stepper_EnPin_Control(X_AXIS, DISABLE);
  if((DISABLE_Y) && (y_active==0)) Stepper_EnPin_Control(Y_AXIS, DISABLE);
  if((DISABLE_Z) && (z_active==0)) Stepper_EnPin_Control(Z_AXIS, DISABLE);
...
//  printf("FAN_HEAD_0:%d ",tail_fan_speed);
#endif
#ifdef AUTOTEMP
  getHighESpeed();
#endif
#ifdef BARICUDA
  #if HEATER_HEAD_1_USED && HEATER_HEAD_1_USED>-1
  analogWrite(HEATER_HEAD_1_USED, tail_valve_pressure);
  #endif

  #if HEATER_HEAD_2_USED && HEATER_HEAD_2_USED>-1
  analogWrite(HEATER_HEAD_2_USED, tail_e_to_p_pressure);
  #endif
#endif
}
...
//LEO   lcd_update();  //////////////////////////////////////
  ...
  target[E0_AXIS] = round(e*axis_steps_per_unit[E0_AXIS]);
#ifdef PREVENT_DANGEROUS_EXTRUDE
  if(target[E0_AXIS] != position[E0_AXIS])
  {
    if(Heater_GetCurrentCelsius((Heater_TypeDef)active_extruder)<EXTRUDE_MINTEMP && !allow_cold_extrude)
    {
      position[E0_AXIS] = target[E0_AXIS]; //behave as if the move really took place, but ignore E part
      printf(SERIAL_ECHO_START);
      printf(MSG_ERR_COLD_EXTRUDE_STOP);
    }
  #ifdef PREVENT_LENGTHY_EXTRUDE
    if(labs(target[E0_AXIS]-position[E0_AXIS]) > axis_steps_per_unit[E0_AXIS]*EXTRUDE_MAXLENGTH)
    {
      position[E0_AXIS] = target[E0_AXIS]; //behave as if the move really took place, but ignore E part
      printf(SERIAL_ECHO_START);
  ...
  block->steps_e = labs(target[E0_AXIS]-position[E0_AXIS]);
  ...
  if(target[E0_AXIS] < position[E0_AXIS]) block->direction_bits |= (1<<E0_AXIS); 
...
#else
  if(block->steps_x != 0) Stepper_EnPin_Control(X_AXIS, ENABLE);
  if(block->steps_y != 0) Stepper_EnPin_Control(Y_AXIS, ENABLE);
#endif
#ifndef Z_LATE_ENABLE
  if(block->steps_z != 0) Stepper_EnPin_Control(Z_AXIS, ENABLE);
#endif
  // Enable all
  if(block->steps_e != 0)
  {
  ...
  delta_mm[E0_AXIS] = ((target[E0_AXIS]-position[E0_AXIS])/axis_steps_per_unit[E0_AXIS]) * extrudemultiply / 100.0;
  if(block->steps_x<=dropsegments && block->steps_y<=dropsegments && block->steps_z<=dropsegments)
  {
    block->millimeters = fabs(delta_mm[E0_AXIS]);
  } 
  else
  {
...
#define MAX_FREQ_TIME (1000000.0/XY_FREQUENCY_LIMIT)
  ...
  if((direction_change & (1<<X_AXIS))==0) x_segment_time[0] += segment_time;
  else
  {
  ...
  if((direction_change & (1<<Y_AXIS))==0) y_segment_time[0] += segment_time;
  else
  {
  ...
  if(speed_factor < 1.0)
  {
    for(i=0; i<4; i++) current_speed[i] *= speed_factor;
  ...
  else
  {
    ...
    if(((float)block->acceleration_st*(float)block->steps_e/(float)block->step_event_count) > axis_steps_per_sqr_second[E0_AXIS])
      block->acceleration_st = axis_steps_per_sqr_second[E0_AXIS];
  ...
  if(fabs(current_speed[E0_AXIS]) > max_e_jerk/2) vmax_junction = min(vmax_junction, max_e_jerk/2);
  ...
  if((moves_queued>1) && (previous_nominal_speed>0.0001))
  {
    ...
    if(fabs(current_speed[E0_AXIS]-previous_speed[E0_AXIS]) > max_e_jerk)
    {
      vmax_junction_factor = min(vmax_junction_factor, (max_e_jerk/fabs(current_speed[E0_AXIS]-previous_speed[E0_AXIS])));
    } 
  ...
  else
  {
    ...
    float advance = (STEPS_PER_CUBIC_MM_E*EXTRUDER_ADVANCE_K) * (current_speed[E0_AXIS]*current_speed[E0_AXIS]*EXTRUTION_AREA*EXTRUTION_AREA) * 256;
    block->advance = advance;
    if(acc_dist == 0) block->advance_rate = 0;
    else block->advance_rate = advance / (float)acc_dist;
  }
#endif
...
void plan_set_position(const float x, const float y, const float z, const float e)
{
  ...
  position[E0_AXIS] = round(e*axis_steps_per_unit[E0_AXIS]);  
  Stepper_SetPosition(position[X_AXIS], position[Y_AXIS], position[Z_AXIS], position[E0_AXIS]);
...
{
  position[E0_AXIS] = round(e*axis_steps_per_unit[E0_AXIS]);  
  Stepper_SetPosition_E(position[E0_AXIS]);
}
...
{
  int8_t i;
  for(i=0; i<NUM_AXIS; i++) axis_steps_per_sqr_second[i] = max_acceleration_units_per_sq_second[i] * axis_steps_per_unit[i];
}
void plan_discard_current_block(void)  
{  
  if(block_buffer_head != block_buffer_tail) block_buffer_tail = (block_buffer_tail + 1) & (BLOCK_BUFFER_SIZE - 1);  
}
...
{
  if(block_buffer_head == block_buffer_tail) return false;
  return true;
}
